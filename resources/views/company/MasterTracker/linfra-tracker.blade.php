@extends('layouts.vertical', ['page_title' => 'Linfra Tracker', 'mode' => session('theme_mode', 'light')])
...
<div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Linfra Tracker</li>
                    </ol>
                </div>
                <h4 class="page-title">Linfra Tracker Management</h4>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card Linfra-card">
                <div class="card-body">
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
                        <h4 class="header-title mb-0">Linfra Tracker Records</h4>
                        <div class="d-flex flex-wrap align-items-center gap-2 ms-auto">
                            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addLinfraTrackerModal">
                            ...
                            <div class="form-floating" style="width: 250px;" height="10px">
                                <input type="text" id="searchLinfraTracker" class="form-control" placeholder=" " aria-label="Search">
                    ...
                    <div class="table-responsive">
                        <table class="table table-centered table-hover dt-responsive nowrap w-100" id="Linfra-tracker-datatable">
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination justify-content-center" id="Linfra-pagination">
...
<!-- Add/Edit Linfra Tracker Modal -->
<div class="modal fade" id="addLinfraTrackerModal" tabindex="-1" aria-labelledby="addLinfraTrackerModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addLinfraTrackerModalLabel">Add New Linfra Tracker Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="addLinfraTrackerForm" method="POST">
                @csrf
...
<!-- Edit Linfra Tracker Modal -->
<div class="modal fade" id="editLinfraTrackerModal" tabindex="-1" aria-labelledby="editLinfraTrackerModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editLinfraTrackerModalLabel">Edit Linfra Tracker Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editLinfraTrackerForm" method="POST">
                @csrf
                @method('PUT')
...
<!-- View Linfra Tracker Modal -->
<div class="modal fade" id="viewLinfraTrackerModal" tabindex="-1" aria-labelledby="viewLinfraTrackerModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewLinfraTrackerModalLabel">Linfra Tracker Record Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row g-3">
...
<div class="modal fade" id="deleteLinfraTrackerModal" tabindex="-1" aria-labelledby="deleteLinfraTrackerModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteLinfraTrackerModalLabel">Confirm Delete</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>Are you sure you want to delete this Linfra Tracker record?</p>
...
<div class="modal fade" id="bulkUploadModal" tabindex="-1" aria-labelledby="bulkUploadModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="bulkUploadModalLabel">Bulk Upload Linfra Tracker Records</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
<div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exportModalLabel">Export Linfra Tracker Data</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="exportForm">
                <div class="modal-body">
...
$(document).ready(function() {
    ...
    let table = $('#Linfra-tracker-datatable').DataTable({
        responsive: true,
        processing: true,
        serverSide: true,
        ajax: {
            url: '/company/MasterTracker/Linfra-tracker/data',
            data: function(d) {
                d.status = $('#filterStatus').val();
                d.category = $('#filterCategory').val();
                d.type = $('#filterType').val();
                d.team = $('#filterTeam').val();
                d.search = $('#searchLinfraTracker').val();
            }
        },
        columns: [
            { data: 'id', name: 'id' },
    ...
    $('#searchLinfraTracker').on('keyup', function() {
    ...
    $('#addLinfraTrackerForm').on('submit', function(e) {
        e.preventDefault();
        
        let formData = new FormData(this);
        
        $.ajax({
            url: '/company/MasterTracker/Linfra-tracker',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                if (response.success) {
                    ...
                    $('#addLinfraTrackerModal').modal('hide');
                    $('#addLinfraTrackerForm')[0].reset();
                    table.ajax.reload();
                }
            },
            error: function(xhr) {
    ...
    $(document).on('click', '.view-record', function() {
        ...
        $.ajax({
            url: `/company/MasterTracker/Linfra-tracker/${recordId}`,
            method: 'GET',
            success: function(response) {
                if (response.success) {
                    ...
                    $('#viewLinfraTrackerModal').modal('show');
                }
            },
            error: function() {
                Swal.fire({
    ...
    $('#editFromView').on('click', function() {
        let recordId = $(this).data('id');
        $('#viewLinfraTrackerModal').modal('hide');
        loadRecordForEdit(recordId);
    });
    ...
    $(document).on('click', '.delete-record', function() {
        ...
        $('#deleteLinfraTrackerModal').modal('show');
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#deleteLinfraTrackerModal').modal('hide');
                    table.ajax.reload();
                }
            },
            error: function() {
    ...
    $('#editLinfraTrackerForm').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#editLinfraTrackerModal').modal('hide');
                    table.ajax.reload();
                }
            },
            error: function(xhr) {
    ...
    $('#bulkUploadForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/company/MasterTracker/Linfra-tracker/bulk-upload',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#exportForm').on('submit', function(e) {
        ...
        let downloadUrl = `/company/MasterTracker/Linfra-tracker/export?${params.toString()}`;
    ...
    $('#downloadTemplate, #downloadTemplateLink').on('click', function(e) {
        e.preventDefault();
        window.open('/company/MasterTracker/Linfra-tracker/template', '_blank');
    });

    // Helper function to load record for editing
    function loadRecordForEdit(recordId) {
        $.ajax({
            url: `/company/MasterTracker/Linfra-tracker/${recordId}/edit`,
            method: 'GET',
            success: function(response) {
                if (response.success) {
                    let record = response.data;
                    populateEditForm(record);
                    $('#editLinfraTrackerModal').modal('show');
                }
            },
            error: function() {
                Swal.fire({
    ...
    $('#addLinfraTrackerModal, #editLinfraTrackerModal').on('hidden.bs.modal', function() {
    ...
    function loadDashboardStats() {
        $.ajax({
            url: '/company/MasterTracker/Linfra-tracker/stats',
            method: 'GET',
            success: function(response) {
                if (response.success) {
